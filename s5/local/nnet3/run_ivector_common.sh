...
if [ $stage -le 3 ]; then
  ...
  mkdir -p exp/chain${nnet3_affix}/diag_ubm
  temp_data_root=exp/chain${nnet3_affix}/diag_ubm
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      ...
      --max-utts 10000 --subsample 2 \
       ...
       exp/chain${nnet3_affix}/pca_transform
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    exp/chain${nnet3_affix}/pca_transform exp/chain${nnet3_affix}/diag_ubm
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/${train_set}_sp_hires exp/chain${nnet3_affix}/diag_ubm exp/chain${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 5 ]; then
  ...
  ivectordir=exp/chain${nnet3_affix}/ivectors_${train_set}_sp_hires
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ...
    exp/chain${nnet3_affix}/extractor $ivectordir
  ...
  for data in ${test_sets}; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk}" \
      data/${data}_hires exp/chain${nnet3_affix}/extractor \
      exp/chain${nnet3_affix}/ivectors_${data}_hires
  done
fi
...
if [ $stage -le 8 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${train_set}_sp data/lang_sp $gmm_dir $ali_dir
fi
